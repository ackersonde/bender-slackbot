...
func CheckCommand(api *slack.Client, slackMessage slack.Msg, command string) {
	...
	if args[0] == "yt" {
		...
		if strings.Contains(response, "york.shire") {
			...
			doAlgoCircleCIURL := "https://circleci.com/api/v1.1/project/github/danackerson/do-algo?circle-token=" + os.Getenv("harhar")
			log.Println("Calling CircleCI build for do-algo: ", doAlgoCircleCIURL)

			log.Println("---------------------------------")
			log.Println(os.Getenv("harhar"))
			log.Println(os.Getenv("CIRCLE_API_TOKEN"))
			log.Println(os.Getenv("slackReportChannel"))
			log.Println("---------------------------------")

			req, err := http.NewRequest("GET", doAlgoCircleCIURL, nil)
			if err != nil {
				// handle err
			}
