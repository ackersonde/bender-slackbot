...
func CheckCommand(api *slack.Client, slackMessage slack.Msg, command string) {
	...
	if args[0] == "yt" {
		...
		if strings.Contains(response, "york.shire") {
			...
			doAlgoCircleCIURL := "https://circleci.com/api/v1.1/project/github/danackerson/do-algo?circle-token=" + os.Getenv("CIRCLE_API_TOKEN")
			log.Println("Calling CircleCI: " + doAlgoCircleCIURL)
			...
			err2 := json.Unmarshal([]byte(content), &data)
			if err2 != nil {
				fmt.Printf("-----------------------------------\n%v\n-----------------------------------\n", err2)
			}
